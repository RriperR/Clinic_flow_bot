...
- просмотр базы знаний по команде `/knowledge`
...
- `KNOWLEDGE_TABLE`, если нужно включить команду `/knowledge`
...
- если `KNOWLEDGE_TABLE` задана, команда `/knowledge` открывает разделы базы знаний из этой таблицы
- если `KNOWLEDGE_TABLE` не задана, команда `/knowledge` отвечает, что база знаний не настроена
...
Если не работает именно `/knowledge`, отдельно проверь `KNOWLEDGE_TABLE` и доступ сервисного аккаунта к этой таблице.
