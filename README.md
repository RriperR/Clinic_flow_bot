...
Если задан `REPORT_CHAT_ID`, бот отправляет в этот чат уведомления о пользовательских действиях и ошибках. В уведомлениях есть `chat_id`, username, ФИО сотрудника из БД при наличии, действие, статус и длительность. Тексты обычных сообщений и `file_id` фотографий в Telegram-лог не отправляются.
