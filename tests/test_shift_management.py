...
async def test_signup_by_shift_id_reports_unavailable_shift() -> None:
    ...
    service = build_shift_service(
        [worker],
        [Shift(id=10, assistant_id=None, doctor_name="Doctor", date=date(2026, 1, 2), type="morning")],
    )

    result = await service.signup_by_shift_id(worker, date(2026, 1, 1), "morning", 10)
...
async def test_signup_by_shift_id_assigns_free_shift() -> None:
    ...
    service = build_shift_service(
        [worker],
        [Shift(id=10, assistant_id=None, doctor_name="Doctor", date=date(2026, 1, 1), type="morning")],
    )
...
async def test_signup_to_doctor_reports_manual_confirmation_when_slots_are_taken() -> None:
    ...
    service = build_shift_service(
        [worker, doctor],
        [Shift(id=10, assistant_id=3, doctor_name="Doctor", date=date(2026, 1, 1), type="morning")],
    )

    result = await service.signup_to_doctor(worker, doctor.id, date(2026, 1, 1), "morning")
